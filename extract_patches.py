...
# 定义过滤白色的函数
def filter_blank(image, threshold = 80): # threshold = 80有点小了，调到100比较合适
...
#     scales = ['10X','20X']
    ...
    # TODO：reading & saving paths
    svs_paths = list(Path('../slides').glob('*.svs'))

        
    svs_paths = svs_paths[args.size*(args.index-1):args.size*args.index]
    patch_path = "../raw_patches"
    ...
    for i,svs in enumerate(svs_paths):  #svs是一个svs图像路径的str
        ...
        #构造存放patch的目录，目录的结构为
        tile_path = svs.replace(str(Path(svs).parent),f'../raw_patches/size{TILE_SIZE}')
        #提取操作
...
#             levels=[level_count-3,level_count-2]   # 10,20
            levels=[level_count-4,level_count-1]   # 5,40
            try:
