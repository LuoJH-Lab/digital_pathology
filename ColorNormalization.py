...
cases = list(Path('../raw_patches/size512').rglob('*.h5'))
SIZE = 512
...
template_file = np.load('../CN_patches/CN_template.npy') # 需要首先挑选一个tile作为CN的模板
...
for case in tqdm(cases_select):
    # 创建对应的CN文件路径
    cn_tiles_path = str(case).replace('../CN_patches/') 
...
with open(f'../CN_patches/unnormed_{args.batch}.pkl','wb') as f:
