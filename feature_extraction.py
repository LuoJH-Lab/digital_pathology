...
#         state_dict = torch.load(f'simclr_feature_extractor/runs/{args.file_path}/checkpoints/model.pth')   #需要修改
    ...
    elif args.extractor == 'retccl_res50_2048': # get features in shape N*2048
...
#     model = model.cuda()
    ...
    # TODO: change the path to the right directory
    patches = list(Path(f'../CN_patches/size512/{args.scale}X').rglob('*.h5'))
    save_dir = '../features/size512/' + f'{args.extractor}'
    ...
    if args.extractor in ['pretrained_resnet0','pretrained_resnet18','pretrained_resnet50','retccl_res50_2048','cTransPath']:
        with torch.no_grad():
            model = model.eval()
